...
processClientMessage(msg)                   ← interceptor.ts
      │
      ├── initialize: rewrites rootUri/rootPath to temp dir
      ├── rewriteUris(params, virtualToReal)  → file:///workspace/main.py
      │                                       → file:///tmp/lsp-xyz/main.py
      ├── didOpen: syncs file content to disk
      ├── didChange: applies incremental edits to disk
...
serverConn.reader.listen(message)           ← jsonrpc.ts line 80
      │
      ├── AUTO_RESPOND check: if server sends a request the client
      │   can't handle (e.g. workspace/diagnostic/refresh),
      │   auto-respond with {result: null} back to server stdin
      │
      ▼
processServerMessage(msg)                   ← interceptor.ts
      │
      ├── rewriteUris(result, realToVirtual) → file:///tmp/lsp-xyz/main.py
      │                                      → file:///workspace/main.py
      └── rewriteUris(params, realToVirtual) → same for notifications
      │
      ▼
...
| [`server/src/lsp/workspace.ts`](packages/server/src/lsp/workspace.ts) | Temp directory management, URI translation |
| [`server/src/lsp/scaffold.ts`](packages/server/src/lsp/scaffold.ts) | Project file scaffolding (Cargo.toml, go.mod, etc.) |
...
| [`server/src/transport/types.ts`](packages/server/src/transport/types.ts) | Transport interfaces, auto-respond method list |
